...
SELECT v.vendor_id, v.vendor_name, COUNT(*) AS booth_rentals
FROM vendor v
INNER JOIN vendor_booth_assignments vba
	ON v.vendor_id = vba.vendor_id
...
c.customer_last_name, SUM(cp.quantity * cost_to_customer_per_qty) AS total_spent 
FROM customer c
INNER JOIN customer_purchases cp
	ON c.customer_id = cp. customer_id
GROUP BY c.customer_id, c.customer_first_name, c.customer_last_name
HAVING SUM(cp.quantity * cost_to_customer_per_qty) > 2000
